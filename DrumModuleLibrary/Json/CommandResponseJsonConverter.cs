...
{
    /// <inheritdoc />
    public class CommandResponseJsonConverter : JsonConverter
    {
        public override bool CanWrite => false;

        private readonly JsonSerializer _basicSerializer;
        private readonly Dictionary<CommandType, Type> _responseTypes;

        public CommandResponseJsonConverter()
        {
            _basicSerializer = new JsonSerializer();
            _responseTypes = typeof(DrumModuleCommandResponse).GetInheritedClasses().ToDictionary(GetCommandType, t => t);
        }

        private CommandType GetCommandType(Type drumType)
        {
            return drumType.GetCustomAttributes(false).OfType<CommandTypeAttribute>().Single().CommandType;
        }
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            if (!jObject.TryGetValue("commandType", StringComparison.InvariantCultureIgnoreCase, out var typeToken))
                throw new Exception($"Input json object doesn't contain 'type' field \r\n {jObject}");

            var typeCode = (CommandType)typeToken.Value<int>();
            if (!_responseTypes.TryGetValue(typeCode, out var type))
        ...
        {
            return objectType == typeof(DrumModuleCommandResponse);
        }
